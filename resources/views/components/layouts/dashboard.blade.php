...
<body class="bg-1 font-sans">
    ...
    <script>
        // Check if user is authenticated by checking if auth header exists
        function checkAuthentication() {
            // Try to make a simple request to check session
        ...
        window.addEventListener('load', function() {
            ...
            if (performance.navigation.type === performance.navigation.TYPE_BACK_FORWARD) {
                // User arrived here via back/forward button, check auth
                checkAuthentication();
            }
        });
        ...
        window.addEventListener('pageshow', function(event) {
            // Check if page is loaded from cache (bfcache)
            if (event.persisted) {
                // Page was restored from cache, check auth
                checkAuthentication();
            }
        });
